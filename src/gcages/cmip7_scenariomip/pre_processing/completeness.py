...
from gcages.cmip7_scenariomip.pre_processing.constants import (
    ...
    REQUIRED_MODEL_REGION_VARIABLES_GRIDDING,
    REQUIRED_MODEL_REGION_VARIABLES_INPUT,
    REQUIRED_WORLD_VARIABLES_GRIDDING,
...
def get_required_world_index_gridding(
    world_region: str = "World",
    region_level: str = "region",
    variable_level: str = "variable",
    variables: tuple[str, ...] = REQUIRED_WORLD_VARIABLES_GRIDDING,
) -> pd.MultiIndex:
    """
    Get the required index for gridding data at the world level in the input
...
def get_required_model_region_index_gridding(
    model_regions: tuple[str, ...],
    region_level: str = "region",
    variable_level: str = "variable",
    variables: tuple[str, ...] = REQUIRED_MODEL_REGION_VARIABLES_GRIDDING,
) -> pd.MultiIndex:
    """
    Get the required index for gridding data at the model region level in the input

    We assume that all combinations of variables
    and model regions need to be included.
