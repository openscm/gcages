...
@functools.cache
def load_ar6_infilling_db(filepath: Path, cfcs: bool) -> pd.DataFrame:
    """
    ...
    if cfcs:
        ...
        res.index = res.index.set_levels(  # type: ignore # pandas-stubs confused
            ...
            level="variable",
        ).set_levels(  # type: ignore # pandas-stubs confused
            res.index.levels[res.index.names.index("unit")].map(  # type: ignore # pandas-stubs confused
                lambda x: x.replace("HFC245fa", "HFC245ca")
            ),
            level="unit",
        )
...
def infill_scenario(
    ...
    # The fact that this is effectively hidden in silicone is an illustration
    ...
    if (
        ...
        infilled_silicone = concat(
            [
                infilled_silicone.loc[~isin(variable=["Emissions|CO2|AFOLU"])],
                (
                    indf.loc[isin(variable=["Emissions|CO2"])]
                    .subtract(
...
def get_ar6_infiller(
    ...
    db = load_ar6_infilling_db(db_file, cfcs=cfcs)
...
class AR6Infiller:
    ...
    def from_ar6_config(  # noqa: PLR0913
        ...
        infillers["Emissions|CO2|Energy and Industrial Processes"] = partial(
            ...
            db_file=ar6_infilling_db_file,
            cruncher=VARS_DB_CRUNCHERS["Emissions|CO2|Energy and Industrial Processes"][
                1
            ],
            cfcs=False,
        )
